...
/** כפתור "פנה בוואטסאפ" — רושם referral דרך ה-API ופותח את wa_link.
 *  הנקודה היחידה שחושפת מספר; דורש request_id (מודל הפרטיות). */
export function ContactWhatsapp({
  ...
  const contact = async () => {
    ...
    setError(null);
    try {
      const res = await postReferral(requestId, developerId);
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      {error && <p className="mt-2 text-center text-xs text-syntax-rose">{error}</p>}
    </div>
  );
}
